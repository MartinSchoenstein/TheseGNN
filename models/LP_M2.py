#!/home/schoenstein/.conda/envs/gnn/bin/python
#SBATCH --job-name=LP_M2_2
...
# --------------  NOT READY -----------------    
if settings["options"]["negative_sampling"] == "cc":
    ...
    cut_val = 0.1
    cut_test = 0.8
    cc_train = cc[: int(len(cc)*cut_train)]
    cc_val = cc[int(len(cc)*cut_train) : int(len(cc)*(cut_train + cut_val))]
    cc_test = cc[int(len(cc)*(cut_train + cut_val)) :]
    cc_train_nodes = []
    for s in cc_train:
        for n in s:
            cc_train_nodes.append(n)
    cc_val_nodes = []
    for s in cc_val:
        for n in s:
            cc_val_nodes.append(n)
    #G_train = G.subgraph(cc_train_nodes).copy()
    #data_train = from_networkx(G_train)
    data_train = data.subgraph(cc_train_nodes) #remplace les deux au dessus normalement NE MARCHE PAS
    negs_train = []
    neg_inside_count = 0
    for c in cc_train:
        #G2_train = G_train.subgraph(c).copy() #mais ducoup il manque G_train ici...
        #G2_train = G.subgraph(c).copy() option 1
        #data2_train = from_networkx(G2_train) option 1 
        data2_train = data.subgraph(list(c)) #option 2
        neg_train_inside = negative_sampling(edge_index = data2_train.edge_index,
                                    num_neg_samples =  int(data2_train.edge_index.size(1) * 1.5))
        neg_inside_count = neg_inside_count + neg_train_inside.size(1) // 2
        negs_train.append(neg_train_inside)
    ratio_neg_inside = neg_inside_count/(data_train.edge_index.size(1) // 2)
    print("Ratio neg intra cc : ", ratio_neg_inside)
    neg_train_outside = negative_sampling(edge_index = data_train.edge_index,
                                        num_neg_samples = int(data_train.edge_index.size(1) * (1 - ratio_neg_inside)))

    train_pos_edge_index = data_train.edge_index
    train_neg_edge_index = torch.cat(negs_train + [neg_train_outside], dim=1)
    ...
    #G_val = G.subgraph(cc_val_nodes).copy()
    #data_val = from_networkx(G_val)
    data_val = data.subgraph(cc_val_nodes)
    negs_val = []
    neg_inside_count = 0
    for c in cc_val:
        G2_val = G_val.subgraph(c).copy()
        data2_val = from_networkx(G2_val)
        neg_val_inside = negative_sampling(edge_index = data2_val.edge_index,
                                    num_neg_samples =  int(data2_val.edge_index.size(1) * 1.5))
        neg_inside_count = neg_inside_count + neg_val_inside.size(1) // 2
        negs_val.append(neg_val_inside)
    ratio_neg_inside = neg_inside_count/(data_val.edge_index.size(1) // 2)
    print("Ratio neg intra cc : ", ratio_neg_inside)
    neg_val_outside = negative_sampling(edge_index = data_val.edge_index,
                                        num_neg_samples = int(data_val.edge_index.size(1) * (1 - ratio_neg_inside)))
    val_pos_edge_index = data_val.edge_index
    val_neg_edge_index = torch.cat(negs_val + [neg_val_outside], dim=1)
    ...
    for s in cc_test:
        for n in s:
            cc_test_nodes.append(n)
    cc_test_nodes = []
    for s in cc_test:
        for n in s:
            cc_test_nodes.append(n)
    #G_test = G.subgraph(cc_test_nodes).copy()
    #data_test = from_networkx(G_test)
    data_test = data.subgraph(cc_test_nodes)
    negs_test = []
    neg_inside_count = 0
    for c in cc_test:
        G2_test = G_test.subgraph(c).copy()
        data2_test = from_networkx(G2_test)
        neg_test_inside = negative_sampling(edge_index = data2_test.edge_index,
                                    num_neg_samples =  int(data2_test.edge_index.size(1) * 1.5))
        neg_inside_count = neg_inside_count + neg_test_inside.size(1) // 2
        negs_test.append(neg_test_inside)
    ratio_neg_inside = neg_inside_count/(data_test.edge_index.size(1) // 2)
    print("Ratio neg intra cc : ", ratio_neg_inside)
    neg_test_outside = negative_sampling(edge_index = data_test.edge_index,
                                        num_neg_samples = int(data_test.edge_index.size(1) * (1 - ratio_neg_inside)))

    test_pos_edge_index = data_test.edge_index
    test_neg_edge_index = torch.cat(negs_test + [neg_test_outside], dim=1)
...
if settings["options"]["attributes"] == "unique":
    ...
    val_data.x = train_data.x.clone()
    test_data.x = train_data.x.clone()
elif settings["options"]["attributes"] == "statistics_all":
    neg_edges = torch.cat([train_data.edge_label_index[:, train_data.edge_label == 0], val_data.edge_label_index[:, val_data.edge_label == 0], test_data.edge_label_index[:, test_data.edge_label == 0]], dim = 1)
    G.add_edges_from(neg_edges.t().tolist())
    ...
    val_data.x = train_data.x.clone()
    test_data.x = train_data.x.clone()
elif settings["options"]["attributes"] == "statistics_allnon":
    degree = dict(G.degree())
    max_degree = max(degree.values())
    degree_norm = {n: d/max_degree for n, d in degree.items()}
    clustering = nx.clustering(G)
...
now = datetime.now()
output_path = settings["output"] + "LP_M2_" + str(now.day) + "-" + str(now.month) + "-" + str(now.year) + "_" + str(now.hour) + ":" + str(now.minute) + ":" + str(now.second) + ".txt"
    ...
    for epoch in range(1, 50):
        ...
        if val_auc > best_val_auc:
            ...
            if count >= limit:
                ...
                break
    output.write(str(time.time() - start))
    output.write("\n")
    output.write("\n")
    output.write("\n")
